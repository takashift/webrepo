...
var (
	...
	conn, dberr = dbr.Open("mysql", "rtuna:" + mysqlPassword + "@unix(/usock/mysqld.sock)/Webrepo", nil)
	...
	e.POST("/agree_signup", func(c echo.Context) error {
		...
		for i := 0; i < len(domain); i++ {
			...
			if strings.HasSuffix(eDomain, domain[i]) {
				...
				d := gomail.Dialer{Host: "smtp.eve.u-ryukyu.ac.jp", Port: 587, Username: "e145771@eve.u-ryukyu.ac.jp", Password: mysqlPassword}
